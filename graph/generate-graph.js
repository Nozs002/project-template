...
function processYAMLRelationships() {
  for (const [id, node] of nodes.entries()) {
    ...
    if (Array.isArray(yaml.documents)) {
      yaml.documents.forEach((docPath) => {
    ...
    if (Array.isArray(yaml.depends_on)) {
      yaml.depends_on.forEach((targetId) => {
        edges.push({
          source: id,
          target: String(targetId).trim(),
          relation: 'depends_on',
    ...
    if (Array.isArray(yaml.implemented_by)) {
      yaml.implemented_by.forEach((targetId) => {
        edges.push({
          source: id,
          target: String(targetId).trim(),
          relation: 'implements',
    ...
    if (Array.isArray(yaml.tested_by)) {
      yaml.tested_by.forEach((targetId) => {
        edges.push({
          source: id,
          target: String(targetId).trim(),
          relation: 'tested_by',
